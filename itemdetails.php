...
if(isset($_POST['submit'])) {
	...
	$checkbidsql = "SELECT item_id, max(amount) AS highestbid, count(id) AS number_of_bids FROM
	...
	//if no bids returned, this bid is the first one, have to comapare with the starting price.
	...
	$inssql = "INSERT INTO bids(item_id, amount, user_id) VALUES(". $validid. ", " . $_POST['bid']
		. ", " . $_SESSION['USERID']. ");";
	mysql_query($inssql);
...
//have to check what mktime does.
$nowepoch = mktime();
$rowepoch = $itemrow['dateepoch'];

//checking if the time now is still behind the max time. if yes, bidding is valid
...
//selecting image to be displayed in the description of item
$imagesql = "SELECT * FROM images WHERE item_id = " . $validid . ";";
$imageresult = mysql_query($imagesql);
$imagenumrows = mysql_num_rows($imageresult);


//select no of bidders and maximum bid tracking by item id.
$bidsql = "SELECT item_id, MAX(amount) AS highestbid, COUNT(id) AS number_of_bids FROM bids WHERE item_id="
...
if($bidnumrows == 0) {
	echo "<strong>This item has had no bids</strong>- <strong>Starting Price</strong>: " . $config_currency
	...
	echo "<strong>Number Of Bids</strong>: ". $bidrow['number_of_bids'] . "
		- <strong>Current Price</strong>: " . $config_currency. sprintf('%.2f', $bidrow['highestbid']);
}

echo " - <strong>Auction ends</strong>: ". date("D jS F Y g.iA", $rowepoch);

//displaying image for the item
echo "</p>";
...
//to bid, a user should be logged in
...
$historysql = "SELECT bids.amount, users.username FROM bids,
	users WHERE bids.user_id = users.id AND item_id = ". $validid . " ORDER BY amount DESC";
